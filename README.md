...
- `slack-agent.*` — **produced by slack-agent**, consumed by the backend: `offboarding.triggered`, `offboarding.cancellation_requested` (BE-7, replaces the old `PATCH .../cancel`), `interview.started`, `interview.turn_recorded` (SA-16, per-turn), `interview.completed`, `dossier.generation_requested`, `sop.creation_requested`, `sop.candidate_offered` (SA-16), `sop.candidate_decided` (SA-16).

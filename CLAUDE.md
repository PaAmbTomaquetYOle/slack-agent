...
- **Interview turns live in memory, but persist incrementally via Kafka (SA-16).** `InterviewService` keeps the live conversation in `IInterviewSessionStore` (`InMemoryInterviewSessionStore`) and crosses over to Kafka at `interview.started`, on every appended turn via `interview.turn_recorded`, and at `interview.completed` (which still carries every turn as a full-set reconciliation backstop — the backend's `interview_turns` table is the actual durable store). On restart, `#resolveSession` rehydrates the in-memory session from the backend's interview read model (`IInterviewRepository.findByProcessId` → `GET /offboarding/{id}/interview`) instead of losing in-flight turns or starting a duplicate interview.
- **SOP candidates persist via a new backend aggregate (SA-16).** `SopService`'s `#candidates` cache is still the source of truth for in-flight offer/eviction bookkeeping, but every offer and decision is also published (`sop.candidate_offered`/`sop.candidate_decided`) to a dedicated backend `SopCandidate` aggregate, so an author's Yes/No click still resolves after a restart. `sopService.rehydrate()` (called once at startup, alongside `orchestrator.recover()`) loads candidates still awaiting a decision from `GET /sop-candidates` via `HttpSopCandidateRepository`.
